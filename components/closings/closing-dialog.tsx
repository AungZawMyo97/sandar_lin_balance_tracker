...
import { createDailyClosingAction } from "@/app/actions/closing";
    ...
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <form action={dispatch} className="space-y-4">
          ...
          {actualBalance && (
            <div
              className={`p-3 rounded text-sm flex justify-between items-center ${isSurplus ? "bg-green-50 text-green-700" : isDeficit ? "bg-red-50 text-red-700" : "bg-gray-100"}`}
            >
              <span>Difference:</span>
              <span className="font-bold">
                {diff > 0 ? "+" : ""}
                {diff.toLocaleString()} {account.currency}
              </span>
            </div>
          )}
