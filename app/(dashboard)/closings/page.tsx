          ...
          <Table>
            ...
            <TableBody>
              {history.map((record) => {
                ...
                const profit = record.profitPerDayMMK
                  ? Number(record.profitPerDayMMK)
                  : null;
                return (
                  <TableRow key={record.id}>
                    <TableCell>
                      {format(record.closingDate, "MMM d, HH:mm")}
                    </TableCell>
                    <TableCell>
                      {record.account.name} ({record.account.currency})
                    </TableCell>
                    <TableCell>
                      {Number(record.systemBalance).toLocaleString()}
                    </TableCell>
                    <TableCell>
                      {Number(record.actualCashBalance).toLocaleString()}
                    </TableCell>
                    <TableCell
                      className={
                        diff > 0
